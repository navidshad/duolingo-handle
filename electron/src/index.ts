...
import { BaseEvent, CloseToolEvent, OpenSubtoolEvent, RouteMessageEvent } from '../../vue_src/src/types/event';
...
const windowsManagerService = new WindowsManagerService(process.env.BASE_URL, MAIN_WINDOW_PRELOAD_WEBPACK_ENTRY);
...
windowsManagerService.onMessage((data) => {
  ...
  if (RoleEvent.instanceof(data as BaseEvent)) {
    const roleEvent = data as RoleEvent;
    if (roleEvent.toolType == 'none') return;

    closeAllTools()
    windowsManagerService.createWindow(roleEvent.toolType)
  }
  ...
  else if (CloseToolEvent.instanceof(data)) windowsManagerService.closeWindow((data as CloseToolEvent).id);
  ...
  // Any windows listening to this channel will receive the message.
    ...
    windowsManagerService.sendGlobalMessage(routeMessageEvent.channelId, routeMessageEvent.data);
  }
})
